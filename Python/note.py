...
def user_action(notebook): 
    while True:
        print(
            '\nДобро пожаловать в заметки\n1 - Просмотр всех заметок | 2 - Добавить заметку |'
            ' 3 - Внести изменения | 4 - Удалить заметку | 5 - Поиск заметок по дате | 0 - Выйти из приложения\n')
        ...
        if user_choice == '1':
            ...
            print('Некорректный выбор! Пожалуйста повторите выбор!')
            print()
            continue


def print_all_notes(file_path):
...
def add_new_note(file_path):  
    with open(file_path, 'a', newline='', encoding='utf-8') as file:
        ...
        if len(file_path) == 0:
            writer.writerow(
                ['ID Заметки', 'Название заметки', 'Содержание заметки', 'Дата/время создания',
...
def edit_note(file_path): 
    ...
    with open(file_path, 'r', newline='', encoding='utf-8') as file, \
        ...
        for row in reader:
            if row[0] == note_id:
                note_title = input("Введите новое название: ")
...
def search_notes_by_date(file_path): 
    ...
    with open(file_path, 'r', newline='', encoding='utf-8') as file:
        ...
        for row in reader:
            if row[3].startswith(target_date):
                print(
                    f"ID заметки: {row[0]}, Название: {row[1]}, Дата/время создания: {row[3]}, Дата/время изменения: {row[4]}")
                print(f"Текст заметки: {row[2]}")
                print("-" * 80)
                found_notes = True
...
def search_note_by_id(file_path, note_id): 
    with open(file_path, 'r', newline='', encoding='utf-8') as file:
        reader = csv.reader(file, delimiter=';')
        header = next(reader)
        for row in reader:
...
try:
    ...
    with open(file_name, 'w', newline='', encoding='utf-8') as notebook:
        ...
        writer.writerow(['ID Заметки', 'Название заметки', 'Содержание заметки',
